...
const PCharacters = ({setPChars, game}) => {
  ...
  return (
    ...
    <div className={styles.charPadding}>
      {charState[0] && <span>
        {star1.map(character => (<Character setPChars={setPChars} game={game} char={character} key={character}/>))}
      </span>}
      {charState[1] && <span>
        {star2.map(character => (<Character setPChars={setPChars} game={game} char={character} key={character}/>))}
      </span>}
      {charState[2] && <span>
        {star3.map(character => (<Character setPChars={setPChars} game={game} char={character} key={character}/>))}
      </span>}
    </div>
    </>
  );
