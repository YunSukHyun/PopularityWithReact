...
const GCharaters = ({game}) => {
  ...
  return (
    ...
    <div className={styles.charPadding}>
      {charState[0] && <span>
        {male.map(character => (<Character game={game} char={character} key={character}/>))}
      </span>}
      {charState[1] && <span>
        {female.map(character => (<Character game={game} char={character} key={character}/>))}
      </span>}
    </div>
    </>
  );
